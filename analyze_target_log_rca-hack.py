...
def process_crashlist_log_tc(tc_no, target_report, cdcsv, base_folder, target_name, run_id, bin_file):
    ...
    if target_name in ['libcoap-server', 'libnyoci-plugtest', 'riot-native-nanocoap-server', 'riot-native-gcoap-server', 'contiki-native-erbium-plugtest', 'mongoose-server', 'coapp-server']:
        ...
        file_name = []
        line_no = []
        for frame in reversed(stack_list):
            function_name.append(frame.get('func', ''))
            file_name.append(frame.get('fullname', ''))
            line_no.append(frame.get('line', ''))
        # try:
        #     #nanocoap hack: i = i+1
        ...
        cdcsv.write( "%s\n%s\n%s\n%s\n" % (tc_no, '\t'.join(function_name), '\t'.join(file_name), '\t'.join(line_no)) )
...
def process_crashlist_log(crashlist_logfile=None, target_report={}):
    ...
    cdcsv = open('%s/%s/%s/cd2.csv' % (crashlist_logfile.split('/')[0], target_info['target_name'], target_info['run_id']), 'w')
    target_report = {}
    ...
    # table_report = get_report(target_report)
    # print table_report
    # with open('%s/%s/%s/cd2_summary.csv' % (crashlist_logfile.split('/')[0], target_info['target_name'], target_info['run_id']), 'w') as f:
    #     f.write(str(table_report))
    cdcsv.close()
