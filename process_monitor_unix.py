...
USAGE = "USAGE: process_monitor_unix.py"\
        ...
        "\n    [-h|--host ipv4]             Host to bind this agent to" \
...
class DebuggerThread:
    def __init__(self, start_command, env):
        """
    ...
    def spawn_target(self):
        ...
        if self.env:
            #print self.env
            self.pid = subprocess.Popen(self.tokens, env=self.env).pid
        else:
    ...
    def stop_target(self):
        try:
            ...
            self.procmon.log("target process already dead")
            pass
        self.alive = False
...
class NIXProcessMonitorPedrpcServer(pedrpc.Server):
    def __init__(self, host, port, cbin, coredump_dir, level=1):
        ...
        self.stop_commands  = []
        self.proc_name      = None
        self.time_to_settle = TIME_TO_SETTLE
    ...
    def start_target(self):
        ...
        self.dbg = DebuggerThread(self.start_commands[0], self.env)
        self.dbg.spawn_target()
        self.dbg.procmon = self
        # prevent blocking by spawning off another thread to waitpid
        t = threading.Thread(target=self.dbg.start_monitoring)
        t.daemon = True
        t.start()
        self.log("done. target up and running, giving it %f seconds to settle in." % self.time_to_settle)
        time.sleep(self.time_to_settle)
        return True
    ...
    def set_time_to_settle(self, time_to_settle):
        self.log("updating target time_to_settle to '%f'" % time_to_settle)

        self.time_to_settle = time_to_settle

    def set_env(self, env):
        self.log("updating target env to '%s'" % env)
        ...
        if env:
            ...
            try:
                if 'PATH' in env:
                    target_env['PATH'] = "%s:%s" % (env['PATH'], target_env['PATH'])
                    del env['PATH']
            except KeyError:
                pass
...
if __name__ == "__main__":
    ...
    opts = None
    try:
        opts, args = getopt.getopt(sys.argv[1:], "c:h:p:l:d:", ["crash_bin=", "host=", "port=", "log_level=", "coredump_dir="])
    ...
    # Signal Handler defined here so we can use closure if needed
    ...
    servlet = NIXProcessMonitorPedrpcServer(host, port, crash_bin, coredump_dir, log_level)
