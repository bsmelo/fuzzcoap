...
##################################################################################################
    ...
    def setup(self, target_name):
        ...
        self.info[target_name]['active_options'] = ['bits', 'bytes']
        ...
        self.fuzz_models[target_name]['bits'] = OrderedDict()
        self.fuzz_models[target_name]['bytes'] = OrderedDict()
        ...
        for pkt in pcap_pkts:
            if pkt.dport == 5683:
                self.fuzz_models[target_name]['bits'][str(model_id)] = [ CorruptedBits(pkt['CoAP']), K_MUT ]
                self.fuzz_models[target_name]['bytes'][str(model_id)] = [ CorruptedBytes(pkt['CoAP']), K_MUT ]
                model_id += 1

        self.info[target_name]['total_active_models'] += len(self.fuzz_models[target_name]['bits']) + len(self.fuzz_models[target_name]['bytes'])
